    ...
    while IFS='=' read -r key value; do
        case "$key" in
            ''|'#'*)
                continue
                ;;
            JWT_SECRET|TURSO_USERS_URL_HTTP|TURSO_USERS_TOKEN|TURSO_CALORIES_URL_HTTP|TURSO_CALORIES_TOKEN|TURSO_EXPENSES_URL_HTTP|TURSO_EXPENSES_TOKEN|TURSO_HEAT_URL_HTTP|TURSO_HEAT_TOKEN)
                if [ -z "${!key}" ]; then
                    export "$key=$value"
                fi
                ;;
...
APP_ENV=$(prompt_value APP_ENV "APP_ENV" false "serverless")
...
JWT_SECRET=$(prompt_value JWT_SECRET "JWT_SECRET" true "")
...
put_param "$SSM_PREFIX/runtime/JWT_SECRET" "$JWT_SECRET"
