terraform {
  backend "s3" {
    key            = "environments/serverless-next/terraform.tfstate"
    encrypt        = true
...
locals {
  ...
  runtime_ssm_parameters = {
    ...
    AUTH_COOKIE_NAME                    = "/${local.ssm_prefix}/runtime/AUTH_COOKIE_NAME"
    AUTH_COOKIE_SECURE                  = "/${local.ssm_prefix}/runtime/AUTH_COOKIE_SECURE"
    AUTH_COOKIE_SAMESITE                = "/${local.ssm_prefix}/runtime/AUTH_COOKIE_SAMESITE"
    AUTH_SESSION_IDLE_SECONDS           = "/${local.ssm_prefix}/runtime/AUTH_SESSION_IDLE_SECONDS"
    AUTH_SESSION_ABSOLUTE_SECONDS       = "/${local.ssm_prefix}/runtime/AUTH_SESSION_ABSOLUTE_SECONDS"
    AUTH_SESSION_ROTATE_AFTER_SECONDS   = "/${local.ssm_prefix}/runtime/AUTH_SESSION_ROTATE_AFTER_SECONDS"
    AUTH_SESSION_ROTATION_GRACE_SECONDS = "/${local.ssm_prefix}/runtime/AUTH_SESSION_ROTATION_GRACE_SECONDS"
    AUTH_SESSION_TOUCH_SECONDS          = "/${local.ssm_prefix}/runtime/AUTH_SESSION_TOUCH_SECONDS"
...
module "static_hosting" {
  ...
  distribution_comment           = "Trackstack serverless-next static frontend + Go API"
