...
- **Milestone reached:** the home, calories, expenses, and heat dashboards now load their authenticated read models in the browser after auth bootstrap, so they no longer depend on `getCurrentUserId()` or SSR request-local auth context.
