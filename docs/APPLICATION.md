...
- **Solid expenses flow:** `/expenses` now owns live dashboard mutations for close-sheet, checklist completion, and entry deletion, and `/expenses/new` now posts real expense entries through the typed OpenAPI client before navigating back to the dashboard.
