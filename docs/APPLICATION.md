...
- **Solid heat route UX:** the `apps/web-next/src/features/heat/{index,new}.tsx` screens now use the same page rhythm as expenses and calories: compact header chrome, one primary overview/details surface, list-based supporting history, and end-of-form save actions instead of a full-width one-off CTA.
