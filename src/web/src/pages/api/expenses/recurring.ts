...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const template = await expensesService.upsertRecurringTemplate({
      id: data.id,
    ...
    console.error("Error in POST /api/expenses/recurring:", error);
...
export const DELETE: APIRoute = async ({ request }) => {
  try {
    ...
    const deleted = await expensesService.deleteRecurringTemplate(data.id, getCurrentUserId());
    if (!deleted) {
    ...
    console.error("Error in DELETE /api/expenses/recurring:", error);
    return new Response(JSON.stringify({ error: "Server Error" }), {
      status: 500,
      headers: {
        "Content-Type": "application/json"
