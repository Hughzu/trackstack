...
Backend-owned helper for creating or updating a test user in the `users` Turso database for `apps/server`.
...
- Uses the `apps/server` Go password hashing implementation so seeded credentials match rebuild auth
