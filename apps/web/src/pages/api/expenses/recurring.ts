...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const template = await fetchApi("/expenses/recurring", {
      method: "POST",
    ...
    console.error("Failed to proxy expenses recurring POST:", error);
...
export const DELETE: APIRoute = async ({ request }) => {
  try {
    ...
    await fetchApi(`/expenses/recurring?id=${encodeURIComponent(id)}`, { method: "DELETE" });
    return new Response(null, { status: 204 });
  } catch (error) {
    console.error("Failed to proxy expenses recurring DELETE:", error);
    return new Response(JSON.stringify({ error: "Server Error" }), {
      status: 400,
      headers: {
        "Content-Type": "application/json"
